<template>
	<span>
		<span v-if="item">
			...
			<a v-on:click="preencheFormulario()" v-if="tipo == 'link'" href="#" data-toggle="modal" v-bind:data-target="'#' + nome" v-bind:class="css || ''">{{titulo}}</a>
		</span>
		<span v-if="!item">
			...
			<a v-if="tipo == 'link'" href="#" data-toggle="modal" v-bind:data-target="'#' + nome" v-bind:class="css || ''">{{titulo}}</a>
		</span>		
	</span>
</template>
